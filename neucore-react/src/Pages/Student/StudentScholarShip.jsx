...
export default class StudentPaymentList extends React.Component {
  ...
  assignScholarshipData = (values, selectedStudentForTransport) => {
    if (values.scholarshipId != "" && selectedStudentForTransport.length > 0) {
      let requestData = new FormData();

      requestData.append(
        "scholarshipId",
        values.scholarshipId != "" && values.scholarshipId != null
          ? values.scholarshipId.value
          : 0
      );
      requestData.append(
        "branchId",
      ...
      assignScholarshipToStudent(requestData).then((response) => {
      ...
      MyNotifications.fire({
        ...
        icon: "error",
        title: "Error",
        msg: "Please give Scholarship input",
        is_timeout: true,
        delay: 1000,
      });
    }
  ...
  getStudentlistforTransport = (
    ...
    requestData.append(
      "scholarshipId",
      scholarshipId != "" && scholarshipId != null ? scholarshipId.value : 0
    );
    // requestData.append(
    //   "standardId",
    ...
    getStudentListforScholarShip(requestData).then((response) => {
      let res = response.data;
      console.log("res", res);
      if (res.responseStatus == 200) {
      ...
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          let d = res.responseObject;
          {
            ...
            this.setState({ opScholarShipList: Opt });
          }
        }
      })
      .catch((error) => {
            ...
            <Formik
              ...
              validationSchema={Yup.object().shape({
                ...
                academicYearId: Yup.object()
                  ...
                  .nullable(),
                scholarshipId: Yup.object()
                  .required("scholarshipId is Required !")
                  .nullable(),
              })}
                    ...
                    <Row style={{ padding: "8px" }}>
                      ...
                      <Col lg={2}>
                        <Form.Group className="createnew">
                          <Form.Label>Student Standard</Form.Label>
                          <Select
                            // className="selectTo formbg"
                            styles={customStyles}
                            isClearable={true}
                            onChange={(v) => {
                              setFieldValue("scholarshipId", "");

                              if (v != null) {
                                setFieldValue("scholarshipId", v);
                              }
                            }}
                            name="scholarshipId"
                            options={opScholarShipList}
